...
## Plot of frog survival for each translocation - grouped by site
...
frog_translocation %>% 
  ...
  inner_join(cohort_survival, by = c("site_id", "date" = "release_date")) %>% 
    ggplot(aes(x = lbdload_med, y = median_survival, color = donor, group = donor)) + 
      ...
      scale_color_manual(values = palette_cbb)
```
* Across all translocated frogs, the 3 donor sites have similar bd_loads. A plot of The 3 donor sites differ somewhat in bd_load of translocated cohorts. 
