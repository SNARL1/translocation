...
- Reduced model is helpful because it reduces the number of values used to calculate probabilities from predictive models.  
...
p_sex <- d1_malec %>% 
  ...
  ggplot(aes(x = male, y = surv)) +
    ...
    annotate(geom = "text", x = 0.4, y = 0.94, hjust = -1, label = "C", size = 4) +
    theme_classic()
...
p_donor <- d1_donor %>% 
  ...
  ggplot(aes(x = donor, y = surv)) +
    ...
    annotate(geom = "text", x = 3.3, y = 0.83, hjust = 0, label = "D", size = 4) +
    theme_classic()
...
### Create multiplot conditional effects figure

```{r}
cond_effects_plot <- (p_snowt1 | p_elev) / (p_sex | p_donor)
```
...
This plot shows frog survival of all translocations grouped by site, and indicates that survival is highly variable across sites, but repeatable within sites. The following analysis quantifies the association between survival in the first translocation and survival in subsequent translocations.
