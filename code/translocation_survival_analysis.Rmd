...
-   Survival is highly variable across sites, but relatively constant within a site.
...
d2 <- d2 %>% 
  filter(transno == 1) %>% 
  group_by(site_id) %>% 
  mutate(survival_trans1 = mean(survival)) %>%
  ungroup(site_id) %>% 
  distinct(site_id, survival_trans1) %>% 
  mutate(survival_trans1 = round(survival_trans1, 3)) %>% 
  inner_join(d2, by = c("site_id")) %>% 
  filter(transno != 1) %>% 
  relocate(translocation_id, .after = site_id) %>% 
...
d1_elevz %>% 
  ...
  ggplot(aes(x = elevation, y = surv)) +  #   "ggplot(aes(x = (elev_z * 2*sd_elev) + mean_elev, y = surv)) +" gives identical result
        ...
        # geom_point(aes(y = .epred)) +
          labs(x = "Elevation of recipient site (meters)", y = "Probability of 1-year survival")
...
p_elev <- d1_elevz %>% 
  ...
  #         labs(x = "Elevation of recipient site (meters)", y = "P(1-year survival)") +
  #         annotate(geom = "text", x = 2500, y = 0.94, label = "b", size = 4, fontface = "bold") +
  #         theme_classic()
  ggplot(aes(x = elevation)) + 
          geom_ribbon(aes(ymin = lower, ymax = upper), fill= "lightgray", alpha = 1) +
          geom_line(aes(y = median), color = "blue", linewidth = 1, alpha = 1) +
          ...
          labs(x = "Elevation of recipient site (meters)", y = "P(1-year survival)") +
          annotate(geom = "text", x = 2400, y = 1, label = "b", size = 4, fontface = "bold") +
          theme_classic()
...
p_snowt1 <- d1_snowt1z %>% 
  ...
  #         geom_ribbon(aes(ymin = lower, ymax = upper), fill = "lightgray", alpha = 1) +
  #         geom_line(aes(y = median), color = "blue", linewidth = 1, alpha = 1) +
  #         labs(x = "Winter severity in year t+1\n(% of average snow depth)", y = "P(1-year survival)") +
  #         annotate(geom = "text", x = 157, y = 0.92, label = "a", size = 4, fontface = "bold") +
  #         theme_classic()
  ...
  ggplot(aes(x = snow_t1)) +
          ...
          coord_cartesian(ylim = c(0, 1.0)) +
          labs(x = "Winter severity in year t+1\n(% of average snow depth)", y = "P(1-year survival)") +
          annotate(geom = "text", x = 7, y = 1, label = "a", size = 4, fontface = "bold") +
          theme_classic()
...
p_sex <- d1_malec %>% 
  ...
  #   labs(x = "Sex", y = "P(1-year survival)") +
  #   annotate(geom = "text", x = 0.4, y = 0.94, hjust = -1, label = "d", size = 4, fontface = "bold") +
  #   theme_classic()
  ggplot(aes(x = male, y = surv)) +
    geom_boxplot(aes(y = .epred, x = sex), fill = "lightgray") +
    coord_cartesian(ylim = c(0, 1.0)) +
    ...
    labs(x = "Sex", y = "P(1-year survival)") +
    annotate(geom = "text", x = 0.15, y = 1, hjust = -1, label = "d", size = 4, fontface = "bold") +
    theme_classic()
...
p_donor <- d1_donor %>% 
  ...
  #   labs(x = "Donor population", y = "P(1-year survival)") +
  #   annotate(geom = "text", x = 3.3, y = 0.94, hjust = 0, label = "c", size = 4, fontface = "bold") +
  #   theme_classic()
  ggplot(aes(x = donor, y = surv)) +
    geom_boxplot(aes(y = .epred, x = donor), fill = "lightgray") +
    coord_cartesian(ylim = c(0, 1.0)) +
    labs(x = "Donor population", y = "P(1-year survival)") +
    annotate(geom = "text", x = 0.5, y = 1, hjust = 0, label = "c", size = 4, fontface = "bold") +
    theme_classic()
