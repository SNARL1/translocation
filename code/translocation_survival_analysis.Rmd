...
### Posterior analysis
```{r}
tidy(m1d_reduce, conf.int = TRUE, conf.level = 0.90)

b2 <- c("(Intercept)", "snowt1_z", "elev_z", "male_c0.548", "donor70567", "donor72996")

mcmc_areas(m1d_reduce, area_method = "equal height", prob = 0.90, pars = b2) +
  geom_vline(xintercept = 0, color = "red", linetype = "dashed", lwd = 0.5) +  
...
d1_elevz %>% 
  ...
  summarise(lower=quantile(.epred, alpha), 
            ...
            med=quantile(.epred, 0.5)) %>%
  ggplot(aes(x=elevation)) + 
          geom_line(aes(y = med), color = "blue", size = 1, alpha = 0.8) +
...
d1_snowt1z %>% 
  ...
  add_epred_draws(m1d_reduce, ndraws = 100, re_formula = NA) %>% 
  ggplot(aes(x = snow_t1, y = surv)) +
    geom_line(aes(y = .epred, group = .draw), color = "blue", size = 0.2, alpha = 0.5) +
#    geom_point(aes(y = .epred)) +
  labs(x = "Winter severity in year following translocation (% of average snow depth)", y = "Probability of 1-year frog survival") 
```
...
d1_malec %>% 
  ...
  ggplot(aes(x = male, y = surv)) +
    geom_boxplot(aes(y = .epred, x = sex)) +
#    geom_point(aes(y = .epred)) +
    labs(x = "Sex", y = "Probability of 1-year frog survival")
```
