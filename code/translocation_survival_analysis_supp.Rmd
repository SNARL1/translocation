...
d1 %>% 
  ...
  ggplot(aes(x = period, y = log10(bd_load + 1))) +
    geom_boxplot() + 
    labs(x = "Period (relative to translocation)", y = (expression(Bd~load~(log["10"]~copies+1)))) +
    coord_cartesian(ylim = c(-0.5, NA)) +
    facet_grid(. ~ site_id1) +
    geom_hline(yintercept = 5.78, color = "red", linetype = 2) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
          strip.text.x = element_text(size = 8),
          panel.grid = element_blank()) +
    geom_text(data = smplsize, aes(x = period, y = -0.4, label = n), size = 2.2, vjust = 1)
