...
Run the following 4 chunks only if you have access to the project database. For those without the necessary credentials, use the saved output file in data/maps/ that is available via the "import-map-data1" chunk.
...
Run the following 4 chunks only if you have access to the project database. For those without the necessary credentials, use the saved output file in data/maps/ that is available via the "import-map-data2" chunk.

### Create dataset of site points

```{r}
donor_sites <- read_csv(here::here("data", "clean", "frog_translocation_final.csv")) %>% 
  distinct(donor) %>% 
  rename(site_id = donor) %>% 
...
bbox <- st_bbox(c(xmin = 245000, ymax = 4231000, xmax = 308000, ymin = 4151000), 
...
translocation_points_map <- tm_shape(hillshade_yose_crop) + 
  ...
  tm_symbols(size = 0.7, # increase point size, use darker blue
             col = "type", 
             palette = c("#008B00", "#C0FF3E"), # tmap only recognizes basic color names (e.g., "blue" but not "lightblue") - use HEX instead
             title.col = "Population type") +
  tm_layout(legend.position = c("right", "top"),
            legend.bg.color = "white") +
  tm_compass(type = "4star",
             position = c(0.80, 0.10),
             size = 2) +
