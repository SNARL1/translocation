...
Evolutionary changes following disease-induced declines are known from a diversity of wildlife populations [@savage2016; @epstein2016; @gignoux-wolfsohn2021; @holland2022].
However, linking these selective responses directly to population recoveries - as necessary for evolutionary rescue - is difficult and remains rare.
In addition to promoting recovery of remnant populations after disease-induced declines, the resistant/tolerant individuals <!--# How best to refer to resistance/tolerance? --> in recovering populations can also be used to reestablish populations that were extirpated following disease emergence [e.g., @mendelson2019].
...
Third, the fitness benefits gained through selection for increased resistance are maintained beyond the natal environment and are maintained across generations, allowing for the establishment of reintroduced populations.
...
Specifically, we used exome capture methods to show that MYL frogs have undergone substantial evolutionary change associated with Bd exposure and that may provide a genetic basis for the observed increased resistance/tolerance to Bd infection observed in recovering populations [@knapp2016].
...
These recovering populations persist in an enzootic state [@briggs2010], characterized by high Bd prevalence and, in adults, moderate Bd infection intensities ("load") that are typically well below the level expected to cause mortality [@knapp2011, @vredenburg2010].

We used two complementary approaches to identify regions of the genome under selection.
...
Second, we used a splined window approach to identify larger genomic regions showing differentiation between population types in *F~ST~* and nucleotide diversity ($\pi_{diff}$ = $\pi_{naive}$ - $\pi_{recovering}$).

The SNP analysis identified 11 outlier SNPs from 7 distinct genes across 4 contigs (@fig-selectionresults C, D: SI, SI Dataset 1).
...
Interestingly, this region had lower $\pi_{diff}$ in the recovering populations, consistent with a pattern of balancing selection.
...
In *Xenopus*, four of these genes occur in the extended MHC Class I region (FLOT1, TUBB, MDC1,TCF19) and four occur in the extended MHC Class III region (HSP70, LSM2, VARS <!--# Check on number of genes, add missing gene here -->) [@ohta2006].
Therefore, we conclude that this candidate region under selection is a part of the extended MHC Class I and III complex and shows synteny with other amphibian genomes.

In summary, our genomics results show that recovering populations have signatures of strong selection on multiple regions of the genome.
Although the functions of all associated genes were not clearly interpretable, regions of the genome under selection in recovering populations included those associated with resistance/tolerance in other amphibian systems, such as the MHC complex.
...
Each of the `r n_distinct(d1$site_id, d1$date)` reintroductions involved collection of adult frogs from 1 of `r n_distinct(d1$donor)` recovering, Bd-positive "donor" populations (@fig-yosemap SI) [@knapp2016] and translocating them to 1 of `r n_distinct(d1$site_id)` nearby recipient sites [@joseph2018; @seddon2014].
...
Of the `r nrow(d2)` translocated populations, `r frog_counts_gt0_nrow` (`r frog_counts_gt0_nrow / nrow(d2)`) showed evidence of successful reproduction in subsequent years, as indicated by the presence of tadpoles and/or subadults.
...
When we decreased $\omega$ from 0.3 to a conservative estimate of 0.15, 50 year extinction probability remained less than 0.5 for three populations and was 0.79 - 0.94 for three additional populations (@fig-viability B).
...
In addition, five of the seven populations translocated after 2013 were predicted to have long-term viability, compared to only one of five populations translocated prior to 2013 (@tbl-survival-earlylate).
...
Recovery in this context is likely to be particularly challenging because typically recovery will need to occur in the presence of ongoing disease.
One potential strategy, and the one we employ in the current study, utilizes populations of resistant/tolerant individuals as a source of animals with which to reestablish extirpated populations in the presence of disease.
...
In particular, although numerous studies have documented signatures of selection in hosts following disease-induced declines [e.g., @epstein2016; @gignoux-wolfsohn2021], we are unaware of any that used individuals bearing the resulting adaptive alleles to recover populations.
Here, we provide evidence that (i) MYL frogs have experienced strong selection following exposure to a recently-emerged lethal fungal disease (Bd) and that some of the genomic regions under selection are associated with genes related to disease resistance/tolerance, and (ii) reintroduction of individuals from these resistant/tolerant populations allows the successful reestablishment of viable frog populations despite ongoing Bd infection.
...
Nonetheless, in the following summary of our results, we use evolutionary rescue as an informative, but tentative framework.
...
These differences in Bd load of frogs from naive and recovering populations are also observed in controlled laboratory studies [@knapp2016, Figure 4].
...
Importantly, some regions under selection are associated with cellular and immunological mechanisms known to contribute to disease resistance, including in amphibians [@grogan2018, reviewed in @zamudio2020].
For example, the major histocompatibility complex (MHC) plays an important role in immunity. In our study, we identified a region that shows evidence of selection in recovering populations and contains eight genes associated with either the MHC Class I or Class III regions. These results corroborate numerous previous studies linking MHC genes to amphibian resistance/tolerance against Bd \[e.g., @savage2011; @bataille2015\].
...
Finally, the RIN3 gene showed a consistent pattern of allele frequency differences across all nine of the frog populations sampled in this study, indicating consistent selection across a wide geographic area.
We provide additional details on these and other genes identified in our study as under selection in the SI.

Reintroductions are an increasingly used method to address escalating loss of wildlife species and populations.
However, recent reviews suggest that such efforts generality have a low success rate (Griffiths Pavajeau 2008, Dodd 2005).
Of the 24 translocations of MYL frogs that we conducted as part of the current study, x% showed evidence of successful reproduction, x% of recruitment of new adults, and x% showed high annual survival and a high probability of persistence over 50 years.
Following translocation, Bd loads were relatively unchanged.
In addition, survival/, low-moderate Bd loads extend beyond the natal environment, maintained between generations.
Success was even higher for more recently conducted translocations.
The xx% success rate for these more recent translocations is particularly remarkable given that population establishment is occurring in the presence of Bd, a pathogen to which MYL frogs are highly susceptible.

several lines of evidence indicate that the selection-derived resistance/tolerance and increased fitness in the presence of Bd is associated with population recovery.
Results from frog translocations conducted as part of the current study indicate that following translocation Bd loads in frogs changed little and post-translocation survival was relatively high at most sites.
These results are in contrast to the outcome of translocations we conducted previously of naive frogs into Bd-positive habitats (conducted prior to our current understanding of the importance of Bd exposure history on frog susceptibilty).
Following those translocations, frogs developed high Bd loads and experienced high mortality (details in SI <!--# These data are unpublished, so if we want to refer to them we'll have to include the data in the SI. Is this necessary to support this result? I don't see any obvious alternatives. -->).
In addition, in the current translocations increased fitness maintained between generations.

Results from frog translocations conducted as part of the current study indicate that higher fitness in the presence of Bd observed in frogs from recovering populations extends beyond the natal environment.
Specifically, following translocation of frogs from natal to recipient sites, Finally, in translocated populations the higher fitness is also maintained in subsequent generations (@fig-compare_surv_probs).
Collectively, these results indicate that the higher fitness observed in recovering Bd-positive populations is not simply a site effect, but also a frog effects and not only the sites, suggesting that the observed resistance and resulting h

These are that the increased fitness repeated translocations and long-term assessment of naturally-recovering populations demonstrate that fitness benefits of robust phenotypes are maintained beyond the natal environment in the presence of Bd.
Specifically, robust phenotypes in recovered populations have reduced Bd loads, below the level known to cause significant host mortality [@vredenburg2010, @knapp2016].
...
<!--# Comment from Allie that will be helpful in making the case that what we are seeing is evidence of strong selection/evolutionary rescue -->
...
We identified outlier SNPs using two Bonferroni-corrected significance levels: "liberal" = 0.05, "stringent" = 0.01.
In the main text we present results solely based on the stringent set.
...
In the main manuscript, we describe the stringent set of variants.
In addition, the liberal set of variants included 38 outliers (35 SNPs and 3 structural variants) from 30 distinct genes across 16 contigs.
...
Based on the SNP and splined-window analyses , no GO terms were significantly over-represented in the set of annotated genes associated with the outlier SNPs or regions.
...
#| tbl-cap: "Association between the proportion of populations translocated early versus late in the study period (< 2013 or >= 2013, respectively) and probability of survival (< 0.5 or >= 0.5)."
#| echo: FALSE
#| message: FALSE
