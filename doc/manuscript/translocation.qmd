...
Individuals from these rescued populations could also be used to reestablish exinct populations.
...
To persist, affected taxa will often need to adapt to their altered environment or face decline and extinction.
"Evolutionary rescue" describes rapid evolutionary change that increases the frequency of adaptive alleles and thereby restores positive population growth [@carlson2014].
In addition to preventing extinction of declining populations, this process could also result in the seeding of nearby empty habitats and allow population reestablishment and recovery.
Although this overall scenario is potentially of critical importance for the conservation of an ever-growing number of threatened taxa, descriptions of its operation in the wild are lacking.

One consequence of human alteration of the biosphere is an increase in emerging infectious diseases [@jones2008; @fisher2012].
Such diseases pose a severe threat to wildlife populations [@daszak2000], and have caused dramatic declines and extinctions in a wide range of taxa [@hewson2014; @samuel2015; @scheele2019; @cunningham2021].
Amid this global loss of biodiverity, some populations decimated by disease have subsequently recovered to varying extents [@newell2013; @voyles2018; @knapp2016].
...
Specifically, we used exome capture methods to show that MYL frogs have undergone substantial evolutionary change associated with Bd exposure and that might provide a genetic basis for the increased resistance/tolerance to Bd infection observed in naturally recovering populations [@knapp2016].
...
These recovering populations exist in an enzootic state [@briggs2010], characterized by high Bd prevalence and, in adults, moderate Bd infection intensities ("load") that are typically well below the level expected to cause mortality [@vredenburg2010].
(See SI for additional details regarding the study design.)
...
One of the 7 identified genes did not have an associated annotation (LOC108802036<!--# Allie - check for accuracy -->).
...
(See SI for additional description of these results.)
...
=======
Interestingly, this region had a low $\pi_{diff}$ value<!--# Negative value shown in 2A? -->, consistent with balancing selection.
Finally, one shared outlier region on Contig8 contained one outlier SNP (TCF19) and was within 360kb of another outlier SNP (VARS) (@fig-spline-manhattan D, @fig-synteny-plot). This region (854kb from the beginning of the outlier window to the VARS SNP) contained a total of 8 annotated genes.
>>>>>>> 6c9a53ad9790c916250bd7ffe7c0def64698e8e9
...
In addition, 1-year survival was higher for frogs translocated later in the study period than earlier (@tbl-survival-earlylate), which we suggest resulted primarily from our improved ability over time to choose recipient sites with higher habitat quality for *R. sierrae*.
Finally, Bd loads were fairly consistent before versus after translocation and loads were nearly always well below the level indicative of severe chytridiomycosis and associated frog mortality (@fig-bdload-beforeafter, see SI text for additional details) [@joseph2018; @vredenburg2010].
...
Although previously proposed [@mendelson2019; @scheele2021], this strategy has rarely been successfully implemented (see [@joseph2018] for a small-scale example).
...
Similarly, the region with the strongest indication of directional selection (as measured by $\pi_{diff}$) contains the gene GVINP1.
...
We developed a more liberal set of outlier variants using a Bonferroni- corrected significance level of 0.05 and used this set solely for the gene ontology (GO) analysis (see below and SI: dataset S1, S2).
...
Finally, we extracted gene transcripts mapped within each region and retrieved annotation for that region using BLAST (SI: datasets S3, S4).
...
Next, we compared the 30 <!--# Allie - Check for accuracy --> unique outlier genes from the liberal set of outlier variants to the complete set of genes in our target transcriptome to determine if any GO biological functions, molecular functions, or cellular processes were overrepresented.
...
<!--# Make sure that code have been uploaded to this link before manuscript submission. -->
...
CMR surveys allowed esimation of adult survival, adult population size, and recruitment of new adults, and VES provided estimates of tadpole and juvenile abundance.
...
Code to create the datasets and fit models is available from the following GitHub repository: [https://github.com/SNARL1/translocation](https://github.com/SNARL1/translocation).

##### Changes in Bd load following translocation

We analyzed skin swabs using standard Bd DNA extraction and qPCR methods [@boyle2004, see SI for details].
...
=======
Code to create the dataset and fit models is available from the following GitHub repository: [https://github.com/SNARL1/translocation](https://github.com/SNARL1/translocation).
<!--# Does it make more sense to only analyze the model with pit_tag_id as the group-level effect? That is the model that is most analogous to a paired t-test. --> <!--# Add  -->
>>>>>>> 6c9a53ad9790c916250bd7ffe7c0def64698e8e9
...
(Additional details regarding incorporating yearly variability in vital rates and estimating model parameters are provided in the SI.)

##### Model analysis

<<<<<<< HEAD
After parameterizing our model with CMR-estimated adult frog survival probabilities and other known vital rates ([@tbl-param_values]), we performed four analyses (described in detail in the SI).
=======
After parameterizing our model with CMR-estimated adult frog survival probabilities and other known vital rates ([@tbl-param_values] : SI), we performed four analyses (described in detail in the SI).
>>>>>>> 6c9a53ad9790c916250bd7ffe7c0def64698e8e9
...
(Additional details are provided in the SI.) <!--# Add link to Mark's GitHub repo? -->
...
<<<<<<< HEAD
This project was supported by grants from the National Park Service (to R.A.K.), Yosemite Conservancy (to R.A.K.), and National Science Foundation (EF-0723563, to C. Briggs; DEB-1557190, to C. Briggs; and DBI-2120084, to C. Richards-Zawacki).
=======
This project was supported by grants from the National Park Service (to R.A.K.), Yosemite Conservancy (to R.A.K.); and National Science Foundation Grants EF-0723563 (to C. Briggs), DEB-1557190 (to C. Briggs), DEB-2133401 (to M.Q.W), and DBI-2120084 (to C. Richards-Zawacki).
>>>>>>> 6c9a53ad9790c916250bd7ffe7c0def64698e8e9

### Figures

![Evidence for selection on individual variants in recovering MYL frog populations at the landscape scale. For each of the 9 naive and recovering MYL frog populations, pie charts show allele frequencies for the 11 outlier SNPs from 7 distinct genes: (A) LOC108802036, (B) TCF19, VARS, MAP1S, TYRP1, and SRFBP1, and (C) RIN3. Charts are superimposed on a map of the Sierra Nevada study area, with Yosemite, Kings Canyon, and Sequoia National Parks (from north to south) shown in dark gray, and the range boundary for MYL frogs in this portion of the Sierra Nevada shown in light gray. The inset map locates the study area in California.](figures/allele_maps.png){#fig-allelefrequencies width="450"}
...
However, for the other 6 genes we still found a statistically significant signal of selection in 2 of the 3 genetic clusters (containing populations 6--9; @fig-allelefrequencies, @fig-selectionresults A SI).
...
Using these best fit trajectories, we assessed whether our model could qualitatively describe the patterns of recovery in the observed data for population 70550.  Code to replicate the analyses can be found at [https://github.com/SNARL1/translocation](https://github.com/SNARL1/translocation).
...
file: spline_window_shared_outliers.csv <!--# Allie - Check for accuracy  -->
