...
Important for conservation action, selection that promotes recoveries of focal populations can also provide sources of individuals with increased resistance and/or tolerance (hereafter, "resistance") to reestablish populations extirpated following disease emergence [e.g., @mendelson2019].
To date, this potentially important species recovery strategy is rarely utilized in wildlife conservation efforts [see @joseph2018 for a notable example], and studies that evaluate its potential utility are critically needed.
<!--# Roland says: Modestly speaking, when combined with the selection results in the current paper, Joseph and Knapp (2018) does provide an example. -->
...
Third, the fitness benefits gained through selection for increased resistance manifest beyond the natal environment, allowing for the establishment of populations reintroduced into novel environments.
...
In contrast, Bd appears to persist in habitats even in the absence of amphibian hosts [@walker2007], and therefore represents a permanent alteration of invaded ecosystems that amphibians will need to overcome to reestablish persistent populations.

Despite the catastrophic impact of Bd on MYL frogs in which most Bd-naive populations are driven to extirpation following Bd arrival [@vredenburg2010], some frog populations have persisted after epizootics [@briggs2010] and are now recovering [@knapp2016].
...
Here, we report the results of a greatly expanded MYL frog reintroduction effort, a long-term viability model, and an associated analysis to evaluate genomic signatures of selection following MYL frog exposure to Bd.
...
<!--# Add Allie's selection results here -->

#### Reintroductions allow population reestablishment
...
These reintroductions involved the collection of adult frogs from large, Bd-positive donor populations that have recovered in recent years following Bd-caused declines [@knapp2016] and translocating them to nearby recipient sites.
We refer to these reintroductions as "translocations".
During the period `r min(d1$year)`--`r max(d1$year)`, we conducted `r n_distinct(d1$site_id, d1$date)` translocations of frogs to `r n_distinct(d1$site_id)` recipient sites (`r transno_min`--`r transno_max` translocations per site; @fig-yosemap) <!--# How to restart numbering for figures in SI, add "S" prefix? -->.
Each translocated cohort included `r transfrogs_min` to `r transfrogs_max` frogs collected from one of `r n_distinct(d1$donor)` donor populations (@fig-yosemap).
...
As such, the average survival of frogs in the first translocation to a site was an important predictor of survival of frogs in subsequent translocations to that site (@fig-transsurvival-postdens; see **Materials and Methods - Frog translocations** for analysis details).
This suggests an important effect of site characteristics and/or (site \* frog) interactions on frog survival.
In addition, 1-year survival was higher for frogs translocated later in the study period than earlier (\@tbl-viability-earlylate; Table 2) <!--# Why isn't cross-referencing working for this table? -->, which we suggest resulted primarily from our improved ability over time to choose recipient sites with higher habitat quality for *R. sierrae*.
Finally, Bd loads changed little between immediately prior to translocation versus during the one year after translocation (@fig-bdload-beforeafter; see SI for analysis details), and were nearly always well below the level indicative of severe chytridiomycosis and associated frog mortality (5.8 ITS copies on a log~10~ scale; @vredenburg2010; @joseph2018).
...
To identify site and frog-specific drivers of post-translocation frog survival, we developed a multilevel model in a Bayesian framework (see **Materials and Methods - Frog translocations** for analysis details).
...
In addition to Bd load, effects of frog size (length), winter severity in the year of translocation (snow_t), day of year on which a translocation was conducted (day), and translocation order (order_first) were all unimportant (@fig-survival-postdens).
In summary, our analyses show that Bd load is not a predictor of the survival of translocated frogs, consistent with frogs in recovering populations having evolved sufficient resistance and/or tolerance to minimize the adverse effects of Bd infection.
In addition, the small changes in Bd load following translocation and relatively high survival of frogs in a substantial number of translocated cohorts suggests that resistant frogs from recovering populations can maintain the benefits of resistance in non-natal habitats (i.e., low-moderate Bd loads, minimal Bd-induced mortality).
...
This result shows that frog resistance in non-natal habitats is maintained across generations of frogs.
...
![For frogs translocated to each recipient site, Bd loads for the period immediately prior to translocation versus during the 1-year period after translocation. <!--# Remove gray background and gridlines -->](figures/bdload_beforeafter.png){#fig-bdload-beforeafter width="450"}
