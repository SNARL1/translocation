...
One possibility is evolutionary modification that restores positive growth to declining populations ("evolutionary rescue").
...
Such reintroductions could play an important role in biodiversity conservation amidst unprecedented global change.
...
Amid this global loss of biodiverity, some populations previously decimated by disease have subsequently recovered to varying degrees [@newell2013; @voyles2018; @knapp2016].
...
In the SI, we provide additional details on these and other genes identified in our study as under selection<!--# Reference complement genes. See Allie's comment in the SI for more on this.  -->.
...
We developed a more liberal set of outlier variants using a Bonferroni- corrected significance level of 0.05 and used this set solely for the gene ontology (GO) analysis (see below).
...
This MHC gene cluster lies on the same chromosome as the outlier SNP in RIN3 (@fig-spline-manhattan C, @fig-allelefrequencies A).
...
![For frogs translocated to each of the 12 recipient sites, Bd loads for the period immediately prior to translocation versus during the 1-year period after translocation. Bd loads are expressed as the number of ITS1 copies per skin swab, as estimated by qPCR of the Bd ITS1 region. Loads indicative of severe disease are \> 5.8 ITS copies (on a log~10~ scale; see SI text for details). Box plot details are as in @fig-cond-effects.](figures/bdload_beforeafter.png){#fig-bdload-beforeafter}
