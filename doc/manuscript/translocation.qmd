...
This parameter expanded data augmentation allows us to capture the possibility that undetected individuals may have recruited into the adult population [@royle2012parameter]. 
...
$$\epsilon^{(\lambda)}_t \sim N(0, \sigma^{(\lambda)}),$$
for periods $t=1, ..., T$. 
...
$$\beta^{(\phi)} \sim N(0, 1),$$
$$\sigma^{(\phi)} \sim N_+(0, 1),$$
$$\epsilon^{(\phi)}_t \sim N(0, \sigma^{(\phi)}),$$
for time $t=1, ..., T$.

The detection model coefficient vector also received a standard normal prior $\beta^{(p)} \sim N(0, 1)$.

We computed the likelihood of each individual capture history using the forward algorithm, and we estimated the latent states using the forward-backward algorithm [@zucchini2009hidden, @joseph2018]. 
...
Second, we performed a local elasticity analysis on $\lambda$ with respect to parameters \$\\sigma\_{J_1}\$, $\sigma_{J_2}$, $\sigma_{A_R}$, and $F$ to determine how small changes in these parameters could influence the long-run deterministic growth rate of populations (@fig-viability-supp).
