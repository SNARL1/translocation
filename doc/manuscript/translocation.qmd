...
Escalating anthropogenic change is devastating global biodiversity.
To avoid extinction, affected populations will often need to adapt to the altered environment.
The process of rapid evolutionary change that increases the frequency of adaptive alleles and thereby restores positive population growth is termed "evolutionary rescue".
In addition to allowing recovery of extant populations, individuals from rescued populations could also be used to reestablish extirpated populations.
We evaluated this scenario in the once-common mountain yellow-legged (MYL) frog, which is threatened with extinction by the human-facilitated emergence of a lethal fungal pathogen (*Batrachochytrium dendrobatidis*; "Bd").
...
Large-scale reintroduction of frogs from these populations resulted in the establishment of reproducing populations at most recipient sites despite ongoing disease.
...
Collectively, these results provide one of the first examples of how evolutionary rescue and the reintroduction of resistant/tolerant individuals can allow the landscape-scale recovery of disease-impacted species.
...
Although this overall scenario is potentially of critical importance for the conservation of an ever-growing number of threatened taxa, descriptions of its occurrence and effectiveness in the wild are lacking.
...
Amid this global loss of biodiversity, some populations decimated by disease have subsequently recovered to varying extents [@newell2013; @voyles2018; @knapp2016].
...
(See SI Appendix for additional description of these results.)
...
Of these, 9 regions were outliers for both metrics ("shared regions") and 2 of these shared regions also contained one or more outlier SNPs (described above).
...
Each of the reintroductions involved collection of adult frogs from 1 of `r n_distinct(d1$donor)` recovering, Bd-positive "donor" populations and translocating them to 1 of `r n_distinct(d1$site_id)` nearby recipient sites (@fig-yosemap) [@seddon2014].
...
Finally, Bd loads were fairly consistent before versus after translocation and loads were nearly always well below the level indicative of severe chytridiomycosis and associated frog mortality (@fig-bdload-beforeafter, see SI Appendix text for additional details) [@joseph2018; @vredenburg2010].
...
The best model of 1-year frog survival identified several important predictors, but Bd load at the time of translocation was not among them (bd_load; @fig-survival-postdens).
...
Males had somewhat higher survival than females, but this effect was only marginally important (sex_male; @fig-cond-effects, @fig-survival-postdens).
...
The results of our study provide evidence that (1) MYL frogs have experienced strong selection following exposure to a recently-emerged lethal fungal disease and that some of the genomic regions under selection are associated with genes related to disease resistance/tolerance, and (2) reintroduction of individuals from these resistant/tolerant populations allows the successful reestablishment of viable frog populations despite ongoing disease.
...
Also in this analysis, the outlier variant with the lowest p-value (uncharacterized gene LOC108802036) is located adjacent to a type I interferon gene (Np-IFNi2) in another frog genome [@gan2018], and together with GVINP1 further indicates the importance of interferon-related genes in this system.
...
This high susceptibility of metamorphs is documented for numerous species of anurans, and may result from the poorly developed immune system characteristic of this life stage [@humphries2022].
...
We expect that such evolutionary rescue-based translocations will be an essential tool in wildlife conservation in an era of accelerating global change.<!--# Dig deeper into the "difficult" task alluded to above? E.g., what would it take to increase confidence that evolutionary rescue is at play, what alternative explanations are still credible given the available data? -->
...
We collected DNA samples via buccal swabbing [@broquet2007] from 53 *Rana muscosa*/*Rana sierrae* individuals: 24 from four naive populations, and 29 from five recovering populations.
...
To visualize the genomic relationship of our populations we conducted a PCA using the glPCA function in the adegenet R package [@jombart2008].
...
<!--# Make sure that code has been uploaded to this link before manuscript submission. -->
...
We conducted `r transno_min`--`r transno_max` translocations per site (@fig-yosemap) and each translocated cohort included `r transfrogs_min` to `r transfrogs_max` frogs (median = `r transfrogs_median`).
...
For each of the translocated populations, we used the presence of tadpoles and/or juveniles from VES and captures of new recruits (i.e, untagged adults) in CMR surveys to provide two measures of successful reproduction.
...
Code used to create CMR datasets and fit models for each translocated population is available from the following GitHub repository: <https://github.com/SNARL1/cmr-analysis>.
...
The second analysis focused specifically on identifying predictors of post-translocation frog survival using a wide range of variables describing characteristics of frogs, sites, and translocations (predictor variables: Bd load (bd_load), sex (sex_male), frog size (length), site elevation (elevation), winter severity in the year of translocation (snow_t), winter severity in the year following translocation (snow_t1), donor population (donor_72996), day of year on which a translocation was conducted (day), and translocation order (order_first)).
...
(Additional details regarding incorporating yearly variability in vital rates and estimating model parameters are provided in the SI Appendix.)

##### Model analysis

After parameterizing our model with CMR-estimated adult frog survival probabilities and other known vital rates ([@tbl-param_values]), we performed four analyses (described in detail in the SI Appendix).
...
<!--# Add link to Mark's GitHub repo? -->
...
In the comparison of individual outlier variants between naive and recovering populations, we detected consistent allele frequency changes across all genetic clusters for 1 of 7 of the associated genes (RIN3; @fig-allelefrequencies).
...
Swab extracts were analyzed singly instead of in triplicate [@kriger2006].
...
![Evidence that frogs translocated from recovering populations can maintain the benefits of resistance/tolerance in non-natal habitats. For frogs translocated to each of the 12 recipient sites, Bd loads for the period immediately prior to translocation versus during the 1-year period after translocation. Bd loads are expressed as the number of ITS1 copies per skin swab, as estimated by qPCR of the Bd ITS1 region. Loads indicative of severe disease are \> 5.8 ITS copies (on a log~10~ scale; see SI Appendix text for details). Box plot details are as in @fig-cond-effects.](figures/bdload_beforeafter.png){#fig-bdload-beforeafter width="500"}

![Evidence that Bd load is not an important predictor of post-translocation frog survival. For the best model identifying predictors of 1-year frog survival following translocation, estimated posterior density curves and shaded 95% uncertainty intervals for the intercept and all predictor variables. In the Bayesian framework in which the model was developed, variables are considered important predictors if the associated uncertainty interval does not overlap zero (indicated by the dashed red line).](figures/mcmc_areas_m1d.png){#fig-survival-postdens width="350"}
